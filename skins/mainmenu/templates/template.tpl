    ...
    <{foreach item=menu from=$block}>
        <{if $menu.level == 0}>
            ...
            <{if ($xlanguage && (($menu.title|strstr:$myStr) || ($menu.image|strstr:$myStr)) || !$xlanguage)}>
                <a class="menuMain<{if $menu.css}> <{$menu.css}><{/if}><{if !empty($menu.topselected)}> maincurrent<{/if}>"
                ...
                <{if !empty($menu.selected)}>
                    <{foreach item=sub from=$block}>
                        <{if $menu.id == $sub.pid}>
                            <{if ($xlanguage && (($menu.title|strstr:$myStr) || ($menu.image|strstr:$myStr)) || !$xlanguage)}>
                                <a class="menuSub marg5<{if $sub.css}> <{$sub.css}><{/if}><{if !empty($sub.selected)}> maincurrent<{/if}>"
